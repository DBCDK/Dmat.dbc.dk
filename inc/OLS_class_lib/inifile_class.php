...
class inifile {
    ...
    public function __construct($filename) {
        ...
        if ($this->ini_file_array = self::parse_ini($filename)) {
            ...
            $this->error = "Empty or no ini-file found ($filename)\n";
            return false;
        }
    }
    ...
    private function write_value($file_handle, $key, $value, $prefix = '') {
        if (is_array($value)) {
            ...
            $key = $prefix . $key;
            if (is_null($value))
                fwrite($file_handle, $key . ' = null' . "\n"); else if ($value === none)
                fwrite($file_handle, "$key = none\n"); else if ($value === 0)
                fwrite($file_handle, "$key = 0\n"); else if ($value === 1)
                fwrite($file_handle, "$key = 1\n"); else if ($value === false)
                fwrite($file_handle, "$key = false\n"); else if ($value === true)
                fwrite($file_handle, "$key = true\n"); else if (is_string($value))
    ...
    private function parse_reserved_words($val) {
        if (substr($val, 0, 1) == '"' && substr($val, -1, 1) == '"')
            return (string)$val; else if (strtolower($val) === 'null')
            return null; else if (strtolower($val) === 'no')
            return (bool)false; else if (strtolower($val) === 'yes')
            return (bool)true; else if (strtolower($val) === 'false')
            return (bool)false; else if (strtolower($val) === 'true')
            return (bool)true; else if ((string)floatval($val) === $val)
            return floatval($val); else if ((string)intval($val) === $val)
            return intval($val); else return (string)$val;
    }
