...
// search for records without faust
    $select = "select seqno, isbn13, provider, format, title, status, faust, originalxml "
            . "from $tablename "
            . "where provider in ('Pubhub') "
            . "and format = 'eReolen' "
//            . "and seqno in (37228,5510,37241,37884,38160,46267,34666,45649,39341,44737,44876)";
            . "and seqno in (42227)";
//            . "and faust is null ";
    ...
    if ($arr) {
        foreach ($arr as $result) {
            ...
            $sql = "select faust, status, title, format "
                    . "from $tablename "
                    . "where provider = 'Pubhub' "
                    . "and format in ('eReolen','ebib','eReolenKlik') "
...
//        }
                ...
                if ($marc) {
                    ...
                    if (count($faustTitles) > 0) {
                        ...
                        if ($oldfaust != $faustTitle[0]) {
                            echo "ens\n";

                            $cxtm->loadXML($xml);

                            $delmarc = $cxtm->Convert2Marc($oldfaust, 'd', 'eReolenLicensPhus');
                            fwrite($fp, $delmarc);
                            $update = "update $tablename set faust = $1, sent_to_basis = null "
//                                . ",title = $2 "
                                    . "where seqno = $seqno";
